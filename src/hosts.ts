...
# Mirror Repo : https://gitlab.com/viptv/hosts
`;
...
async function findIp(host: string) {
  ...
  $("#dns tr").each((i, element) => {
    let td = $(element).children();

    if ($(td[0]).text() === "A") {
      ipList.push($(element).children().last().text());
    }
  });
...
function updateMd(content: string, config: UpdateConfig) {
  ...
  if (needUpdate) {
    ...
    if (config.mdTpl && config.mdDest) {
      ...
      const replacedMdContent = template
        .replace("{hostContent}", nextHosts)
...
async function saveHosts(hostData: HostData[], config: UpdateConfig) {
  try {
    updateMd(tpl.replace("{content}", buildHosts(hostData)).trim(), config);
  } catch (err) {
